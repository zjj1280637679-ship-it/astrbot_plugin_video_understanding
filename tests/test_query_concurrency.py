...
async def test_same_tool_object_keeps_concurrent_video_queries_isolated(monkeypatch):
    ...
    first_result, second_result = await asyncio.gather(
        tool.call(
            _tool_context(first, first_llm_generate),
            query="FIRST_QUERY",
            video_index=0,
        ),
        tool.call(
            _tool_context(second, second_llm_generate),
            query="SECOND_QUERY",
            video_index=0,
        ),
    )
    ...
    first_token = re.search(token_pattern, first_prompt)
    second_token = re.search(token_pattern, second_prompt)
