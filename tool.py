...
def _provider_request_has_native_video_urls() -> bool:
    try:
...
def build_video_attachment_marker(bound: BoundVideo, video_path: str) -> str:
    ...
    return f"[Video Attachment{quoted}: name {bound.display_name}, path {video_path}]"


def build_video_transport_kwargs(bound: BoundVideo, video_path: str) -> dict:
    if _provider_request_has_native_video_urls():
...
class QueryVideoTool(FunctionTool[AstrAgentContext]):
    ...
    parameters: dict = Field(
        default_factory=lambda: {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "One focused question to answer from the video."},
                "video_index": {"type": "integer", "description": "Video index in the current request. Defaults to 0.", "default": 0, "minimum": 0},
                "time_range": {"type": "string", "description": "Optional focus range such as 00:12-00:25.", "default": ""},
            },
            "required": ["query"],
    ...
    async def call(self, context: ContextWrapper[AstrAgentContext], **kwargs) -> ToolExecResult:
        ...
        if index >= len(bindings):
            return f"VIDEO_QUERY_ERROR: video_index {index} is out of range; available indexes are 0..{len(bindings) - 1}"
        ...
        try:
            ...
            logger.warning("[video-semantic-search] failed to resolve video index=%s", index, exc_info=True)
            return "VIDEO_QUERY_ERROR: AstrBot could not resolve the selected video"

        prompt = build_video_query_prompt(query=query, time_range=str(kwargs.get("time_range") or ""))
        try:
            ...
            logger.warning("[video-semantic-search] provider call failed provider=%s type=%s", provider_id, type(exc).__name__, exc_info=True)
            return "VIDEO_QUERY_ERROR: the configured model could not consume the current AstrBot video input contract"
        ...
        if text == VIDEO_INPUT_UNAVAILABLE:
            return "VIDEO_QUERY_ERROR: the configured model did not receive a usable video through the current AstrBot provider path"
